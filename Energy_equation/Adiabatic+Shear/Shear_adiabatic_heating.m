...
% using pressure-velocity formulation for medium with variable viscosity; 
% computation and visualization of shear and adiabatic heating distribution 
...
% Compute srain rate Exx and stress S'xx
exx=zeros(ynum,xnum);
sxx=zeros(ynum,xnum);
...
fid = fopen('/home/vundar/Documenti/Codice_Fortran/Benchmark/Adiabatic_Shear/data.txt', 'w+');
% Compute srain rates Exy, stress Sxy, shear heating and adiabatic heating
exy=zeros(ynum,xnum);
sxy=zeros(ynum,xnum);
hs=zeros(ynum,xnum);
ha=zeros(ynum,xnum);
...
for i=2:1:ynum-1
  for j=2:1:xnum-1
    % Exy, Sxy
     ...
     ha(i,j)=tk*alpha*gy*rho(i,j)*vy1(i,j);
     fprintf(fid, '%f %f %f %f %d %d', x(j), y(i), hs(i,j), ha(i,j), vx(i,j),vy(i,j));
     fprintf(fid, '\n');
  end
end
fclose(fid);
...
% Plotting shear heating as colormap
subplot(1,3,1);
pcolor(x(2:1:xnum-1)/1000,y(2:1:ynum-1)/1000,hs(2:1:ynum-1,2:1:xnum-1));      % making a colormap
%colormap hot;
...
title('shear heating (W/m^3)'); % title for the plot
xlabel('x');        % title for the horizontal axis
ylabel('y');        % title for the vertical axis
axis ij image ;     % directing vertical axis downward, making proper dimensions
axis([0 xsize/1000 0 ysize/1000]); % Making axes limits
caxis([-1e-5 1e-5]) % Limits for the colorscale

%  Plotting adiabatic heating as colormap
subplot(1,3,2);
pcolor(x(2:1:xnum-1)/1000,y(2:1:ynum-1)/1000,ha(2:1:ynum-1,2:1:xnum-1));      % making a colormap
shading interp;     % making smooth transitions between colors
...
hold off;           % stop plotting on the colormap
box on;             % making a box around the plot
title('adiabatic heating (W/m^3)'); % title for the plot
xlabel('x');        % title for the horizontal axis
ylabel('y');        % title for the vertical axis
axis ij image ;     % directing vertical axis downward, making proper dimensions
axis([0 xsize/1000 0 ysize/1000]); % Making axes limits
...
%  Plotting total heating as colormap
subplot(1,3,3);
pcolor(x(2:1:xnum-1)/1000,y(2:1:ynum-1)/1000,h(2:1:ynum-1,2:1:xnum-1));      % making a colormap
shading interp;     % making smooth transitions between colors
colorbar;           % showing a colorbar for the map
hold on;            % continuing plotting on the colormap 
% Plotting velocity vector as arrows using internal nodes only
...
hold off;           % stop plotting on the colormap
box on;             % making a box around the plot
title('total heating (W/m^3)'); % title for the plot
xlabel('x');        % title for the horizontal axis
ylabel('y');        % title for the vertical axis
axis ij image ;     % directing vertical axis downward, making proper dimensions
axis([0 xsize/1000 0 ysize/1000]); % Making axes limits
